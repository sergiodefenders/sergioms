...
<div class="single_port_navigation"><a href="<?php if($rd_data['rd_bc_portlink']){echo esc_url($rd_data['rd_bc_portlink']);} ?>" class="all_projects_btn"><span class="ap_top_point"></span><span class="ap_bottom_point"></span></a><?php $prev = get_permalink(get_adjacent_post(false,'',false)); if ($prev != get_permalink()) { ?><a href="<?php echo esc_url($prev); ?>" class="next_project"><?php echo __('Siguiente', 'thefoxwp'); ?></a><?php } ?><?php $next = get_permalink(get_adjacent_post(false,'',true)); if ($next != get_permalink()) { ?><a href="<?php echo esc_url($next); ?>" class="previous_project"><?php echo __('Anterior', 'thefoxwp'); ?></a><?php } ?></div>
...
if ($video !== ''){
	echo "<div class='post-attachement'>".do_shortcode($video)."</div>";
}
...
if(get_post_meta($post->ID, 'rd_share_buttons', true) == 'yes' ) { ?>
	...
	if ($p_url !=='') {
		echo '<div class="port_vp">';
				...
				echo '<a href="'.$p_url.'" target="_blank">'.__('Ver Proyecto', 'thefoxwp').'</a>';
...
echo '<div class="port_metas">';
	if ($client !=='') {
		...
		echo '<div class="port_meta clearfix"><div class="port_meta_first">'.__('Cliente', 'thefoxwp').':</div>';
		echo '<div class="port_meta_last">'.$client.'</div></div>';
	}
	if ($p_url !=='') {
		echo '<div class="port_meta clearfix"><div class="port_meta_first">'.__('Link', 'thefoxwp').':</div>';
		$url = preg_replace('#^https?://#', '', $p_url);
				echo '<div class="port_meta_last"><a href="'.$p_url.'" target="_blank">'.$url.'</a></div></div>';
	}
	if ($categories_r !=='') {
		 
		echo '<div class="port_meta clearfix"><div class="port_meta_first">'.__('Categoría', 'thefoxwp').':</div>';
		echo '<div class="port_meta_last">'.$categories_r.'</div></div>';
	}
	if ($tags_r !=='') {
		 
		echo '<div class="port_meta clearfix"><div class="port_meta_first">'.__('Etiquetas', 'thefoxwp').':</div>';
...
<?php }
  ...
  if(get_post_meta($post->ID, 'rd_related_post', true) == 'yes') {?>
    ...
    <div class="single_port_related clearfix">
      <h2><?php echo __('Proyectos Similares', 'thefoxwp'); ?>:</h2>
